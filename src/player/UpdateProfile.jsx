...
export default function UpdateProfile() 
{
  ...
  useEffect(() => {
    ...
    if (storedPlayer) {
      setFormData(JSON.parse(storedPlayer));
    }
  }, []);
  ...
  const handleSubmit = async (e) => 
  {
    ...
    try 
    {
      const response = await axios.put(`${config.url}/player/updateprofile`, formData);
      if (response.status === 200) 
      {
        setMessage(response.data);
        setError('');
        sessionStorage.setItem('player', JSON.stringify(formData)); // Update local session data
      }
    } 
    catch (error) 
    {
